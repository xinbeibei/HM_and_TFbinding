...
outFile <- paste0(outPath, 'summary_auroc.txt') # fixed filename "summary.txt"
...
for (i in 1 : nrow(cmb)){
    ...
    plot(perf, main=paste0(title, " AUROC: ", round(as.numeric(auc@y.values), 4)))
    dev.off()

    r2 <- c(r2, round(as.numeric(auc@y.values), 4))    
}
